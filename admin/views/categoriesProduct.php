<?php $categories = View::$getView;; //получаем список категорий товаров
...
<a class="addCategory" href="/admin?addCategoriesProduct=10">Добавить категорию</a>
...
<!--выводим материалы-->
	<?php for($i;;$i++) : ?>
		...
		?>
		<div class="edit-item">
			<a href="<?php echo '/admin?editCategoriesProduct='.$categories[$i]['id']?>"><?php echo $categories[$i]['title']?></a></h2>
			
			<!--удаление категории-->
			<form class="delete" method="POST" action="<?php echo $uri; ?>">
				<input type="hidden" name="delete-id" value="<?php echo $categories[$i]['id'] ?>" />
				<input type="hidden" name="delete-type" value="categoriesProduct" />
